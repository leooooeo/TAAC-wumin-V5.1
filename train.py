...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="row_group",
        help="Train/valid split: row_group=tail Row Groups as valid (default); "
        "timestamp=int64 Parquet column timestamp quantile split (same column as batch tensors).",
    )
    parser.add_argument(
        "--train_ratio",
        type=float,
        default=1.0,
        help="If data_split=row_group: fraction of train-side Row Groups to use. "
        "If data_split=timestamp: quantile on rows with ts < valid threshold (earlier time subset).",
    )
    parser.add_argument(
        "--valid_ratio",
        type=float,
        default=0.1,
        help="If data_split=row_group: fraction of Row Groups for validation (tail). "
        "If data_split=timestamp: tail fraction by timestamp quantile (valid_ts_threshold).",
    )
    parser.add_argument(
        "--eval_every_n_steps",
        type=int,
...
def main() -> None:
    ...
    logging.info(f"USER_CACHE_PATH = {cache_path}")
    if cache_path and os.path.exists(cache_path):
        files = os.listdir(cache_path)
        logging.info(f"Files in USER_CACHE_PATH:")
        for f in files:
            logging.info(f"  {f}")
    else:
        logging.warning(f"USER_CACHE_PATH not set or path does not exist: {cache_path}")
    ...
    pair_int_feature_specs = build_feature_specs(
        pcvr_dataset.pair_int_schema, pcvr_dataset.pair_int_vocab_sizes
    )
