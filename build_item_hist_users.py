...
-----------------------
  ...
  int_user_scalars.npy(M, 7)      int32   — 7 stable user scalars
  int_user_dense.npy  (M, 256)    float16 — user_dense_feats_61
  meta.json                       — schema info (scalar fids, dense fid/dim, …)
...
def scan_interactions(
    ...
    """Single pass over the training parquets. Returns five flat arrays of
    ...
    needed = scalar_cols + [dense_col, "item_id", "timestamp", "label_type"]
    ...
    user_dense = np.zeros((total_rows, USER_DENSE_DIM), dtype=np.float16)
    ...
    for f in files:
        ...
        for rg in range(pf.metadata.num_row_groups):
            ...
            for i in range(B):
                ...
                if n > 0:
                    user_dense[offset + i, :n] = vals[s:s + n].astype(np.float16)
    ...
    return item_ids, timestamps, labels, user_scalars, user_dense
...
def main() -> None:
    ...
    t0 = time.time()
    item_ids, timestamps, labels, user_scalars, user_dense = scan_interactions(
        files, total_rows
    )
